...
local function display_question_in_buffer(data)
  ...
  -- FIX: Add the blank line and the header as two separate items
  table.insert(lines, "")
...
local function fetch_and_show_question(title_slug)
...
local function show_problems_in_float(problems)
  problem_list_cache = problems
  local buf = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_option(buf, 'bufhidden', 'wipe')
  vim.api.nvim_buf_set_option(buf, 'buftype', 'nofile')

  local lines = { "LeetCode Problems (Press <Enter> to select, q to close):", "---------------------------------------------------------" }
  ...
  vim.api.nvim_buf_set_keymap(buf, 'n', '<CR>', '', {
    ...
    callback = function()
      ...
      if problem_list_cache[problem_index] then
        local slug = problem_list_cache[problem_index].stat.question__title_slug
